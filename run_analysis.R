...
# handle the duplicate colnames , while duplicates ,paste numbers i at the end of the feature
i <- 1
...
labels <-
  read.csv(file = "./UCI HAR Dataset/activity_labels.txt", sep = "", header = FALSE)
...
result <-
  data %>% group_by(activity_labels, subject) %>% summarise_all(mean)
